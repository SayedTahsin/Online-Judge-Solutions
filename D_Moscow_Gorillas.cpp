...
void solve()
{
    ...
    map<int, pii> mp;
    for (int i = 0; i < n; i++)
        mp[a[i]].first = i, mp[b[i]].second = i;

    fore(mp) if (x.second.first > x.second.second) swap(x.second.first, x.second.second);

    int x = mp[1].first;
    int ans = (x * (x + 1)) / 2;
    x = n - mp[1].second - 1;
    ans += (x * (x + 1)) / 2;
    x = mp[1].second - mp[1].first - 1;
    ans += (x * (x + 1)) / 2;

    int l = mp[1].first, r = mp[1].second, c1, c2;
    ...
    fore(mp)
    {
        if (x.first == 1)
            continue;
        int X = x.second.first, Y = x.second.second;
