...
void solve()
{
    ...
    cin >> n;
    vector<pii> v(n);
    fore(v) cin >> x.first >> x.second;
    vector<pair<int, pii>> cop;
    for (int i = 0; i < n; i++)
    {
        cop.pb({v[i].first, {v[i].second, i + 1}});
        cop.pb({v[i].second, {v[i].first, i + 1}});
    }
    sort(all(cop));
    // fore(cop) cout << x.first << ' ' << x.second.first << ' ' << x.second.second << endl;
    // cout << endl;
    vector<int> b(2 * n);
    vector<pii> pre(2 * n);
    int mn = INF;
    ...
    {
        b[i] = cop[i].first;
        if (cop[i].second.first < mn)
        {
            mn = cop[i].second.first;
            idx = cop[i].second.second;
        }
        pre[i].first = mn;
        pre[i].second = idx;
        // cout << b[i] << ' ' << pre[i].first << ' ' << pre[i].second << endl;
    }

    vector<int> ans;
    fore(v)
    {
        int h = x.first, w = x.second;
        int c = lower_bound(all(b), h) - b.begin();
        c--;
        if (c < 0)
        {
            ans.pb(-1);
